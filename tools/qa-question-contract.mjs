...
check("GAS 學習紀錄保存常錯題型與錯誤熱區", /"errorsJson"/.test(gas) && /"errorStatsJson"/.test(gas) && /function normalizeErrorCounts_\(value\)/.test(gas) && /function normalizeErrorStats_\(value\)/.test(gas) && /errorByLane/.test(gas) && /errorByWave/.test(gas) && /topErrors/.test(gas));
...
check("前端教師報表支援班級／關卡篩選、常錯摘要與熱區", /progressClassFilter/.test(html) && /progressStageFilter/.test(html) && /row\.topErrors/.test(html) && /row\.errorByLane/.test(html) && /row\.errorByWave/.test(html) && /state\.errorCounts/.test(html) && /state\.errorStats/.test(html));
