...
const savedErrorRecord = post({ action: "saveRecord", sessionToken: token, record: { stage: 5, mode: "chineseChar", zone: "大橋國小", result: "win", score: 120, correct: 8, attempts: 10, accuracy: 80, errors: { "chineseChar:學": 3, "chineseChar:橋": 1 }, errorStats: [{ mode: "chineseChar", expected: "學", lane: "teacher-left-upper", wave: 3 }, { mode: "chineseChar", expected: "學", lane: "teacher-left-upper", wave: 3 }, { mode: "chineseChar", expected: "橋", lane: "teacher-bottom", wave: 5, phase: 2 }], responseStats: [{ mode: "chineseChar", expected: "學", lane: "teacher-left-upper", responseMs: 820 }, { mode: "chineseChar", expected: "橋", lane: "teacher-right-mid", responseMs: 1180 }] } });
const studentSummary = get("students", { token: "contract-test-admin", limit: 10 });
check("學習紀錄可回傳常錯題型、反應統計與錯誤熱區", savedErrorRecord.ok && studentSummary.ok && studentSummary.students.some(student => student.studentName === "50101" && student.topErrors?.[0]?.key === "chineseChar:學" && student.topErrors[0].count === 3 && student.avgResponseMs === 1000 && student.responseSamples === 2 && student.responseByLane?.some(lane => lane.lane === "teacher-left-upper" && lane.avgResponseMs === 820) && student.errorByLane?.some(lane => lane.lane === "teacher-left-upper" && lane.count === 2) && student.errorByWave?.some(wave => wave.wave === 3 && wave.phase === 0 && wave.count === 2)));
