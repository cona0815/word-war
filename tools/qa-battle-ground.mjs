  ...
  for (const viewport of [{width:1920,height:1080},{width:1366,height:768},{width:800,height:900},{width:390,height:844},{width:320,height:780}]) {
    ...
    for (let stage=0;stage<9;stage++) {
      const result = await page.evaluate(async index=>{
        ...
        return {stage:index+1,heroFoot:h,bossFoot:b,difference:Math.abs(h-b),labelInBounds:tag.left>=0&&tag.right<=innerWidth,heroPositioned:Math.abs(heroBounds.left+heroBounds.width/2-(bounds.left+bounds.width*.32))<1,bossInBounds:bossBounds.left>=bounds.left&&bossBounds.right<=bounds.right,bossCoordinateMatches:Math.abs(bossBounds.left+bossBounds.width/2-(bounds.left+bounds.width*bossPos.x/100))<1,controlsInBounds:[command,hp].every(r=>r.left>=bounds.left&&r.right<=bounds.right),mobileControlsSeparate:innerWidth>900||(tag.bottom<=hp.top&&hp.bottom<=command.top&&command.bottom<=bossBounds.top)};
      },stage);
      assert.ok(result.difference<1,JSON.stringify(result));
      assert.equal(result.labelInBounds,true,JSON.stringify(result));
      assert.equal(result.heroPositioned,true,JSON.stringify(result));
      assert.equal(await page.evaluate(()=>Math.abs(parseFloat(getComputedStyle(gameScreen).getPropertyValue('--hero-x'))-32)<.01&&bossPos.x===68),true,'Hero retreats to 32% while Boss remains at 68%');
