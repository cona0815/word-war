## 2026-09-20：暫停鍵移至戰鬥畫面右上角

- 依畫面回饋，暫停鍵已從側邊選單移除，改為右上角選單鍵下方的常駐戰鬥按鈕。
- 桌機及 390px 手機畫面驗證位置為 top 70px / right 14px；未開始戰鬥時自動隱藏。

## 2026-09-20：玩家暫停遊戲

- 新增暫停遊戲功能；對話框按繼續或 Esc 恢復。
...
- 目視檢查角色與文字；擷取過程無頁面 JS 錯誤。沒有修改遊戲功能或學生資料。

## 2026-09-13 六張真實遊戲宣傳截圖

- docs/promo-six/ 完成六張附箭頭註解的宣傳圖、原始截圖、可編輯 HTML 與主貼文／逐張文案。使用隔離本機展示資料，無正式網路存檔。
- 主題為登入雙主角、校園八景、鍵位方向、國字輸入、八寶石儀式與大橋堂天梯。頁面未報 JavaScript 錯誤，六張已目視檢查。
- 登入實際截圖保留在 raw/01-login.png；新版無介面封面仍為 assets/word-war-game-cover-logo-v2.png。
- 工程待辦維持計畫書的小兵面向與四類課程，未將待辦宣傳為完成。

## 2026-09-13 遊戲封面徽章標題

- 新增 assets/word-war-game-cover-logo-v2.png，依使用者提供的 logo-overlay.png 將標題改為藍金徽章、羽毛、劍與 WASD 鍵帽風格。使用內建 imagegen 編輯，已目視確認標題及兩位人物完整入鏡。
- 保留前版素材；尚未替換登入頁。後續工程仍依 completion-plan-2026-09-13.md。

## 2026-09-13 新需求盤點與遊戲封面

- 完成 docs/completion-plan-2026-09-13.md，優先修正小兵朝向人物，以及單注音／單聲調／國字／詞語四類課程。此為計畫，尚未改動執行期。
- 確認 render() 未校正小兵面向，typing-input.js 現有正則不處理獨立聲調；現有靜態 QA 111/111、題庫契約 60/60 通過，並不代表新需求完成。
- 新增 assets/word-war-game-cover-16x9.png，移除原音樂字樣並改為 DCPS 校園鍵盤勇者；已目視確認，尚未接入登入頁。
- 下一步：M1 小兵原始方向盤點與八方向朝向修正。

## 2026-09-13 橫式音樂封面

- 新增 word-war-cover-16x9.png，男女主角全身合作施法，附 Word War 與 Twin Stars Awaken 標題。使用內建 imagegen，已目視確認雙腳、姿勢與標題。

## 2026-09-13 雙主角全身與站姿修正

- 新增 shot-02-duo-fullbody.png，拉遠構圖，兩人四隻靴子及腳下地面完整可見；改成平衡站姿，書本靠身、施法手自然彎曲。已目視檢查。
- 更新 Kling 描述為保持全身遠景，素材包改用全身版。影片待生成驗收。

## 2026-09-13 開場改為男女主角合作

- 新增 shot-02-duo.png，沿用既有角色，以男女主角同等突出、掌心與水晶法杖合力施法呈現合作。已目視檢查。
- 更新分鏡、Kling 描述詞與雙主角 ZIP；原男主角圖片保留為歷史素材，不納入新版 ZIP。影片仍約 15 秒，尚待 Kling 動態驗收。
- 圖片使用內建 imagegen 編輯，提示詞保存於 docs/kling-opening/duo-image-prompt.txt。

## 2026-09-13 Kling 15 秒開場影片素材包

- 完成 docs/kling-opening 的三鏡首幀、原版透明 Logo、15 秒分鏡表與逐鏡中文影片描述詞。校園與大橋堂沿用遊戲素材，主角鏡頭以既有角色及校園為參考生成，已目視檢查。
- 不修改遊戲程式；Kling 影片尚未生成，動態角色、鍵帽與建築穩定性待影片驗收。
- 下一步：在 Kling 逐鏡生成約 5 秒片段，剪接並於末尾疊上原版 Logo。

# Word War Handoff

## 2026-09-13 外部 Chrome／Edge 原生 IME 阻塞

- 本輪已嘗試用指定的 Chrome 與 Edge 建立獨立外部測試工作階段；目前 CUA 只提供 Codex 內嵌瀏覽器，兩個外部瀏覽器均回覆 unavailable。系統雖有 Chrome／Edge 程序，但沒有可安全取得的控制分頁。
- 使用者已開啟外部 Chrome，但重新盤點後仍只有 Codex 內嵌瀏覽器，外部分頁沒有被連接器暴露；因此不能把「已開啟」當成代理人已取得控制權。
- 因此沒有假稱完成 Windows 原生注音候選字或 Ctrl+C/V 驗收，也沒有注入使用者既有瀏覽器、剪貼簿或其他資料。先前內嵌瀏覽器的第 6／8 關、八顆寶石、大橋堂儀式與 Ctrl+A／Ctrl+Z 結果仍有效。
- 下一步：若外部瀏覽器仍未被連接器暴露，請依進度規格逐項人工記錄原生 IME、標點、Ctrl+C/V 與 30 分鐘課堂結果，再把結果交回工作區。
...
- 結果已保存至 `docs/qa-gas-live/result.txt`；wrapper 結束時清理本機程序環境變數，未把管理密碼或其他秘密寫入檔案。這次只觸碰隔離測試資料，不代表正式班級資料已驗收。
- QA 修正 `finishLadder` 斷言：排行榜保留帳號歷史最佳成績時，回傳暱稱可能是較早紀錄；現在確認回傳項目為 approved 且 floor >= 1，沒有改動執行期規則。
- 剩餘風險：真實 Windows 中文 IME／實體鍵盤、30 分鐘課堂穩定性、高層天梯平衡、垂直 UI 人工驗收與 37 套施法素材。
- 下一個最安全任務：在實體 Windows 鍵盤上做 IME、Ctrl 熱鍵、八景流程與大橋堂儀式人工驗收。
...
- 本輪提交：`ba96229`（GAS／前端錯誤熱區報表）與 `93c3aff`（本機離線彙整對齊雲端欄位）；隔離 GAS 線上端點 9/9 驗收另見上方紀錄。
- 下一個最安全任務：在實體 Windows 鍵盤上驗收，收集實際課堂資料後再調整平衡或擴充圖表。
...
- `tools/qa-progress-report.mjs` 增加全班摘要與篩選後摘要驗收；既有逐關最佳、常錯內容、TSV 匯出仍通過。
- 進度規格將學生分析剩餘項目收斂為反應時間與鍵位／波次熱區；這個摘要只使用已載入的 students API／本機資料，不改動學生存檔。
- 尚缺垂直 UI 人工驗收、真實 Windows IME、線上 GAS 部署、高層天梯平衡、反應時間／鍵位熱區與 37 套施法素材。
- 下一個最安全任務：補記錄每題反應時間與 lane，先以向後相容欄位與只讀統計驗收。
...
- 下一個最安全任務：有隔離 GAS 網址時做線上部署冒煙測試；否則補齊教師題庫 laneKey 欄位說明並進行教室實機驗收。

## 2026-09-13 大橋堂八寶石放置儀式

- `index.html` 的大橋堂現在採兩段式啟動：八顆寶石解鎖第 9 張卡片後，第一次按鈕只完成「字母、單字、注音、符號、國字、操作、英句、中句」八寶石放置；第二次確認才建立天梯戰鬥。
- `tools/qa-daqiao-ritual.mjs` 驗證放置後尚未進入戰鬥、畫面文案與八種寶石齊全，第二次確認才會進入天梯；inline syntax 與既有回歸維持通過。
- 進度規格同步修正八關 Boss 三階段分流、教師管理與學生報表的過期描述；本次未修改既有素材或 `hero-cast-clips.js`。
- 尚缺垂直 UI 人工驗收、真實 Windows IME、線上 GAS 部署、高層天梯平衡、跨班級鍵位／波次熱區與 37 套施法素材。
- 下一個最安全任務：有隔離 GAS 網址時做線上部署冒煙測試；否則進行教室實機輸入與大橋堂儀式／窄版報表欄位人工驗收。

## 2026-09-13 主角 Boss 戰固定置中

- `index.html` 將 Boss 戰主角位置與一般戰鬥統一為 `50%`，主角不再因 Boss 出現跳到 32%；Boss 仍維持右側 68% 的對戰位置，施法朝向與腳底對齊邏輯保留。
- `tools/qa-battle-ground.mjs` 同步改為要求 Boss 戰主角水平置中；五種寬度（1920、1366、800、390、320）九關共 45 組通過，`qa-game` 123/123、Boss 題池／進度／IME／教師與報表回歸也通過。
- 進度規格已將主角置中標為已接入；尚缺垂直 UI 人工驗收、真實 Windows IME、線上 GAS 部署、高層天梯平衡與 37 套施法素材。
- 下一個最安全任務：有隔離 GAS 網址時做線上部署冒煙測試；否則進行教室實機輸入與窄版報表欄位驗收。
...
- 下一個最安全任務：在有隔離 GAS 網址與測試帳號時做線上部署冒煙驗收；沒有部署條件時，進行教師實機輸入與報表欄位人工驗收。
