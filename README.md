...
戰鬥畫面右上角直接提供「暫停」按鈕，不必打開選單；按「繼續遊戲」或 Esc 恢復，怪物、飛行攻擊、道具效果與音樂同步暫停。

## 學生試玩與 Netlify

登入帳號區下方的「先體驗，不用帳號」可直接開始第一關。體驗進度只保留在當次遊戲記憶體，不改學生存檔、不上傳成績；選單可結束體驗回到登入。

體驗版選單可任選八景及大橋堂：第一關從 Lv.1 起步，第二～七關對應 Lv.2～7，第八關 Lv.7，大橋堂 Lv.8；不預先發寶石。正式帳號仍循序解鎖並保留已取得等級。Boss 戰主角退到畫面 32%，小兵戰仍置中；答對 Boss 後約 0.4 秒命中，小兵圖會先解碼並在失敗時使用替代圖示。
...
八景八關都以基礎鍵位、聲調、國字、英文單字及中文詞語為上限；簡單英中句子只在大橋堂。後段逐關提速並可同時四隻敵人。主線最高 Lv.7，大橋堂第 1／3／5 層解鎖 Lv.8／9／10。大招已加入小兵血量傷害並修正 Boss 階段下限。

完整關卡／時間／升級規格：`docs/course-update-2026-09-14.md`。六組 Suno 配樂描述詞：`docs/suno-battle-prompts.md`。新版含約 12.7 秒、約 3 MB 的開場影片，可靜音、略過與重播。

本機預覽：`node tools/serve-preview.mjs`，再開啟 `http://127.0.0.1:8767/`。本機修改尚未部署正式站；GAS 後端也需同步更新。
...
主線共有 8 個校園景點關卡，每關 4 波小怪後進入三階段 Boss 戰，完成時間依作答速度而定。第 1～6 關依序練英文字母、英文小單字、單一注音、單聲調與標點、國字、詞語與熱鍵；第 7 關為 2～6 字母的多樣英文單字，第 8 關為 2～4 字中文詞語。集滿八顆寶石後開啟大橋堂，才加入簡單英中句子。
...
- 怪物靠近中央會扣能量。
- 共用電腦不建議儲存管理密碼；教師可在右上角「選單」→「設定」讀取學生帳號、重設五碼密碼／停用帳號，也可讀取學生進度，依班級／最高關卡篩選並匯出逐關最佳、常錯摘要、平均反應、熱門 lane 與錯誤 lane／波次熱區；管理題庫支援搜尋與篩選題目、編輯題目、管理大橋堂英中句子的難度（舊第 7／8 類代碼及波次欄位保留相容）、停用／重新啟用或軟刪除；新增題庫時臨時輸入即可。
...
- `node tools/qa-progress-report.mjs http://127.0.0.1:8767`：以隔離 Chrome 驗證學生進度班級／關卡篩選、班級摘要、逐關最佳、常錯摘要、錯誤 lane／波次熱區及 TSV 匯出。
- `node tools/qa-daqiao-ritual.mjs http://127.0.0.1:8767`：驗證集滿八顆寶石後，必須先在大橋堂完成放置儀式，第二次確認才會啟動天梯戰鬥。
- `node tools/qa-question-lane.mjs http://127.0.0.1:8767`：驗證教師題目的 `laneKey` 會控制英文／中文句子的出怪座標與同 lane 排隊。
