...
- 怪物靠近中央會扣能量。
- 共用電腦不建議儲存管理密碼；教師可在右上角「選單」→「設定」讀取學生帳號、重設五碼密碼／停用帳號，也可讀取學生進度，依班級／最高關卡篩選並匯出逐關最佳、常錯摘要、平均反應與熱門 lane；管理題庫支援搜尋與篩選題目、編輯題目、指定第 7／8 關波次與難度、停用／重新啟用或軟刪除；新增題庫時臨時輸入即可。
...
- `node tools/qa-progress-report.mjs http://127.0.0.1:8767`：以隔離 Chrome 驗證學生進度班級／關卡篩選、班級摘要、逐關最佳與常錯摘要及 TSV 匯出。
