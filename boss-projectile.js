...
(() => {
  const flights=new Set();let paused=false;
  ...
  async function fire(){
    ...
    flights.add(animation);if(paused)animation.pause();
    try{await animation.finished}catch{return false}finally{flights.delete(animation);bolt.remove()}
  ...
  window.BossProjectile=Object.freeze({fire,
    pause(){paused=true;flights.forEach(animation=>animation.pause())},
    resume(){paused=false;flights.forEach(animation=>animation.play())}
  });
})();
